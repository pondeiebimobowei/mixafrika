      ...
      <ScrollView className="flex-1 px-4" showsVerticalScrollIndicator={false}>
        ...
        <View className="items-center mb-8">
          ...
          <TouchableOpacity className="flex-row items-center bg-[#3f2e05] px-4 py-1.5 rounded-full border border-[#856404]">
            <Text className="text-[#fbbf24] font-bold mr-1">🏆 GOLD</Text>
        ...
        <View className="bg-white dark:bg-[#111827] rounded-3xl p-5 mb-6 dark:border border-gray-800">
          <View className="flex-row items-center gap-2 mb-2">
            <Text className="text-black dark:text-gray-400">Available Balance</Text>
          ...
          <Text className="text-black dark:text-white text-4xl font-bold mb-6">
          ...
          <View className="flex-row gap-3">
            ...
            <TouchableOpacity className="flex-1 bg-[#1f2937] py-3 rounded-xl flex-row items-center justify-center gap-2">
        ...
        {/* Menu Items */}
        <View className="bg-white dark:bg-[#111827] rounded-3xl overflow-hidden mb-8 dark:border border-gray-800">
          {MENU_ITEMS.map((item, index) => (
            <TouchableOpacity
              key={index}
              className={cn(
                "flex-row items-center justify-between p-4 active:bg-gray-800",
                index !== MENU_ITEMS.length - 1 && "border-b border-gray-300 dark:border-gray-800"
              )}
              onPress={() => router.push(item.route as any)}
            >
              <View className="flex-row items-center gap-4">
                <View className="w-10 h-10 rounded-xl bg-white dark:bg-[#1f2937] items-center justify-center">
                  <item.icon size={20} color="#9ca3af" />
                </View>
                <View>
                  <Text className="text-black dark:text-white font-semibold text-base">{item.title}</Text>
                  {item.subtitle && (
        ...
        <View className="items-center mb-10">
          ...
          <Text className="text-gray-600 text-xs mt-4">Version 2.4.1 (Build 2046)</Text>
        </View>
